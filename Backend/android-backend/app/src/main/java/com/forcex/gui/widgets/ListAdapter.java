...
public abstract class ListAdapter <T extends Object> {
	...
	void onTouch(float x,float y,byte type){
		...
		}else if(listview.listener != null && type == EventType.TOUCH_DROPPED && (Math.abs(cursorY - cursorYOld) < 0.04f || dispatchDrag && clickTime < 0.35f)){
	...
	void create(){
		...
		rectcursorx = listview.getExtentWidth() * 0.1f;
	...
	void render(Drawer drawer){
		...
		if(cursorY < 0.0f || items.size() < simpleViewVisible){
			cursorY = 0.0f;
		}
		testCulling();
		dy = (listview.local.y + listview.getExtentHeight()) - item_height + cursorY;
