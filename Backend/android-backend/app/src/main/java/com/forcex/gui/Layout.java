...
public class Layout extends View {
	...
	public static final byte ALIGNMENT_CENTER 		= 0xA;
	public static final byte ALIGNMENT_RIGHT 		= 0xB;
	public static final byte ALIGNMENT_LEFT 		= 0xC;
	public static final byte ALIGNMENT_NONE 		= 0x0;
	...
	public void onTouch(float x, float y, byte type) {
		for(short i = 0;i < children.size();i++){
			if(children.get(i).isVisible() && !children.get(i).ignoreTouch){
				if(children.get(i).testTouch(x,y)){
					children.get(i).dispatchTouch(x,y,type);
					return;
				}
			}
		}
