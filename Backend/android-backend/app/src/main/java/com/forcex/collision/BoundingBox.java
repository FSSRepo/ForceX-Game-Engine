...
public class BoundingBox
{
	...
	public static void create(BoundingBox dest,float[] vertices){
		...
		dest.center.set(dest.max);
		dest.center.addLocal(dest.min);
		dest.center.multLocal(0.5f);
		dest.extent.set(dest.max.x - dest.center.x,dest.max.y - dest.center.y,dest.max.z - dest.center.z);
		dest.extent.absoluteLocal();
		dest.radius = dest.extent.length() * 0.5f;
	}
