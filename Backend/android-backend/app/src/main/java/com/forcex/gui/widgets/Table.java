...
public class Table extends View {
	...
	public void onCreate(Drawer drawer) {
		...
		for(short y = 1;y < dimens[1];y++){
			float offset_width = (extent.x - (cell_width * y));
			put(vertexs,vec,offset_width,-extent.y);
			put(vertexs,vec+1,offset_width,extent.y);
			put(indices,i,vec,vec+1);i++;
			vec += 2;
		}
