...
public class Dialog {
	...
	private float label_width, label_height,
	close_width, padding = 0.01f, icon_dimens;
	private Color background;
	private Vector2f position;
	protected short id = 0;
	static short id_gen = -32000;
	...
	public Dialog(Layout layout){
		...
		id = id_gen;
		id_gen++;
		this_is_priority = layout.context.isDialogEmpyt();
	...
	public void setTitle(String text){
		if(textview == null){
			...
			textview.setTextSize(label_height * 0.6f);
	...
	private void settingDialog(){
		...
		layout.sortViews();
		if(useLabel){
			label_width = (layout.extent.x + padding) - close_width;
			if(label_width <= 0.07f){
				label_width = 0.2f;
			}
			if(icon_texture != -1){
				icon_dimens = label_height * 0.4f;
			}
		}
	}
	...
	void onTouch(float x,float y,byte type){
		if(useLabel){
			...
			if(type == EventType.TOUCH_DROPPED && GameUtils.testRect(x,y,position.add((layout.extent.x + padding) - close_width,(layout.extent.y + padding + label_height)),close_width,label_height)){
				if(listener != null){
					if(listener.dimiss()){
						dimiss();
					}
	...
	public void thisPriority(){
		layout.context.rewindPriority();
	...
	boolean testTouch(float x,float y){
		return state == 1 && GameUtils.testRect(x,y,position.add(0,(useLabel ? label_height : 0.0f) + padding),layout.getExtentWidth() + padding,layout.getExtentHeight() + padding + (useLabel ? label_height : 0.0f));
	}
