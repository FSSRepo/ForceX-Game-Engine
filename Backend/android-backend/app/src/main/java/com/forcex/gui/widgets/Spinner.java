...
public class Spinner extends View implements ListView.onItemClickListener {
	...
	public void onCreate(Drawer drawer) {
		...
		listview.setHeight(extent.y * 2f);
		listview.updateExt();
	...
	public void onDraw(Drawer drawer) {
		...
		if(choose_option){
			listview.local.set(local.x,local.y - extent.y - listview.getExtentHeight());
			listview.onDraw(drawer);
			drawer.setScale(listview.getExtentWidth(),listview.getExtentHeight());
			drawer.renderLineQuad(listview.local, edge_color);
		}else{
			drawer.setScale(extent.x,extent.y);
