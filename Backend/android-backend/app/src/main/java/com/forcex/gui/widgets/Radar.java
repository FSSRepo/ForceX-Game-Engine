...
public class Radar extends View
{
	...
	public void update(Vector3f posplayer, float rotationZ){
		angle = rotationZ;
		player_pos.set(Maths.clamp(posplayer.x / world_x,-1,1),Maths.clamp(posplayer.y / world_y,-1,1));
	}
	...
	private void updateVbo(){
		...
		for(byte i = 0;i < vertexs.length;i += 4){
			Vector2f vert = matrix2.mult(tmp.set(vertexs[i + 2],vertexs[i + 3]));
			if(i == 0){
				Vector2f v = matrix2.mult(tmp.set(1,1));
				cornerX = ((player_pos.x - v.x) * 0.5f) + 0.5f;
				cornerY = -((player_pos.y + v.y) * 0.5f) + 0.5f;
			}
			vertexs[i + 2] = vert.x + cornerX;
			vertexs[i + 3] = vert.y + cornerY;
		}
		if(vbo == -1){
	...
	{
		Vector2f nt = GameUtils.getTouchNormalized(x,y);
		if(type == EventType.TOUCH_PRESSED){
			Vector3f player = new Vector3f(
				((nt.x - local.x) * world_x) / extent.x,
				((nt.y - local.y) * world_y) / extent.y,
				300
			);
			player_pos.set(nt);
			if(listener != null){
				listener.onPressPoint(nt.sub(local),player);
			}
		}
	}
	...
	public static class MapObject{
		...
		public MapObject(boolean dynamic,int icon,Vector3f posi){
	...
	public static abstract class onRadarListener{
		Radar rad;
		public abstract void onPressPoint(Vector2f radar_position,Vector3f world_position);
