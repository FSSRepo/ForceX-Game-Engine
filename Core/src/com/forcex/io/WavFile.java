...
public class WavFile{
	public byte[] data;
	public int samplerate,format,channel,bitrate;
	public float time;
	...
	public float getSampleLenght(int byteoffset){
		int num_samples = (int)(samplerate * rate_timing);
	...
	public float getSample(int byteoffset){
		int real_offset = byteoffset + (samplerate / 2 * (bitrate / 8));
	...
	public String getPlayTime(int byteoffset){
		float timer = (float)byteoffset / (samplerate * channel * bitrate / 8f);
		int t = (int)timer % 60;
		return ((int)timer / 60)+":"+(t < 10 ? "0" : "")+t;
	}
	
	public float getPorcentPlayed(int byteoffset){
		return byteoffset / data.length;
	}
	
	public int getOffsetTime(float time){
		return (byte)(time * (samplerate * channel * bitrate / 8f));
	}
