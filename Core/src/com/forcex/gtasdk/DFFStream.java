...
public class DFFStream {
															...
															case SPECULAR_MAT:
																{
																	...
																	mat.specular_name = is.readString(specularSize - 8);
																	is.skip(4);
				...
				if (is.readInt() == EXTENSION)
				{
					if (is.readInt() != 0)
					{
						...
						if (is.readInt() == COLLISION_MODEL)
						{
							...
							FileOutputStream col = new FileOutputStream(FX.homeDirectory + "zmdl/col.cache");
	...
	private static boolean analizeDff(DFFSDK dff, onDFFStreamingListener listen, LanguageString lang)
	{
		...
		for (byte i = 0;i < dff.frameCount;i++)
		{
			...
			if (test.length() == 0)
			{
				if (listen != null)
				{
					listen.onStreamError(lang.get("no_name_frame"), true);
				}
				return false;
			}
			for (byte j = 0;j < dff.frameCount;j++)
			{
		...
		for(byte i = 0;i < dff.geometryCount;i++){
		...
		try
		{
			...
			for (int i = 0;i < dff.frameCount;i++)
			{
				...
				if (dff.fms.get(i).name.length() > 0)
				{
					...
					frame.writeString(dff.fms.get(i).name);
	...
	private static void writeMaterial(DFFSDK dff, DFFMaterial mat, RWSection matlist)
	{
		...
		if (mat.hasSpecularMat) {
			...
			extension.writeInt(8 + mat.specular_name.length());
	...
	private static void writeExtensionClump(DFFSDK dff, RWSection clump)
	{
		...
		if (dff.hasCollision)
		{
			...
			try
			{
				FileInputStream is = new FileInputStream(FX.homeDirectory + "zmdl/col.cache");
