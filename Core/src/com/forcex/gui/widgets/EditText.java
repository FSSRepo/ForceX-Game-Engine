...
public class EditText extends View {
	...
	public void onDraw(Drawer drawer) {
		...
		if(hint.length() > 0 && text.length() == 0){
			...
			textview.onDraw(drawer);
			if(frame < 30 && using){
				cursor.set((local.x - extent.x) + getXCursor() * 2f,textview.local.y - ((num_lines - 1) * textview.getTextSize()));
	...
	{
		if(password){
			return textview.getTextWidthReal("*") * lines[num_lines - 1].length();
		}
		 return textview.getTextWidthReal(lines[num_lines - 1]);
	}
			...
			if(keyboard.solveShowView()){
				...
				keyboard.setOnKeyBoardListener(new KeyBoard.onKeyBoardListener(){
						...
						public void onKeyDown(char key){
							if(only_numbers){
								if(key == '.' && text.contains(".") || key=='-' && text.contains("-")){ return; }
							}
							text += key;
							updateTextView();
							processChange(false);
