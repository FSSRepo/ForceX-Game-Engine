...
public class HelpTip {
	...
	public HelpTip(UIContext ctx){
		...
		tvbo = ctx.getDrawer().genBuffer(new float[]{
							 0,1f,0,0,
							 -1.5f,0,0,0,
							 0,-1f,0,0
						 },false,false);
		drag_color = new Color(255,255,255,255);
	...
	private void show(View view){
		this.focus = view;
		local = new Vector2f(view.local.x,view.local.y);
		focus.setVisibility(View.INVISIBLE);
	}
	
	public void onTouch(float x,float y,byte type){
		if(type == EventType.TOUCH_DROPPED && GameUtils.testRect(x,y,text.local.set(0.8f,-0.8f),btn_extents.x,btn_extents.y)){
	...
	public void render(Drawer drawer){
		...
		if(!request_show && tips.size() > 0){
			if(current == null){
				...
				show(current.view);
				left = local.x <= 0;
				arrow_from.set(local.x + (focus.getExtentWidth() + 0.15f) * (left ? 1 : -1.0f),local.y);
				arrow_to.set(local.x + (focus.getExtentWidth() * 2f + 0.15f) * (left ? 1 : -1.0f),local.y);
				dragging = false;
				if(current.drag_x){
			...
			if(current.reference_img != -1){
				img_location.x = -0.95f + current.img_width;
	...
	private void renderTitle(Drawer drawer){
		...
		text.local.y = (focus.local.y + focus.getExtentHeight()) < 0.8f ? 0.98f - text.getHeight() : ((focus.local.y - focus.getExtentHeight()) - 0.1f - text.getHeight());
	...
	private void renderDescription(Drawer drawer){
		...
		text.local.set(-0.95f + text.getWidth(),y - 0.05f - text.getHeight());
		text.onDraw(drawer);
	}
}
