...
public class ModernView extends View {
	int vbo,ibo,t2_vbo,t_vbo;
	short index_offset,index_count,segments,triangle_show;
	Color triangle_color,start_color,end_color,power_color;
	...
	public class WrapProp{
		...
		public WrapProp(float lapse,float velocity,boolean inverse){
			this.lapse = lapse;
	...
	public ModernView(float radius,int segments){
		...
		wraps[0]= new WrapProp(0.5f,90f,true);
		wraps[1]= new WrapProp(0.6f,120f,false);
		wraps[2]= new WrapProp(0.2f,150f,true);
		wraps[3]= new WrapProp(0.7f,180f,true);
		wraps[4]= new WrapProp(0.4f,120f,true);
		wraps[5]= new WrapProp(0.1f,360f,false);
		wraps[6]= new WrapProp(0.999f,0f,true);
	...
	public void onCreate(Drawer drawer) {
		...
		int vertexOffset = 0;
		float angle = 0;
		t_vbo = drawer.genBuffer(new float[]{
			2f,1.2f,0,0,
			-2f,1.2f,0,0,
			0.0f,-2f,0,0,
			2f,1.2f,0,0,
		},false,false);
		t2_vbo = drawer.genBuffer(new float[]{
									 0.2f,0.13f,0,0,
									 -0.2f,0.13f,0,0,
									 0.0f,-0.2f,0,0
								 },false,false);
	...
	public void setVelocity(int idx,float velocity){
		wraps[idx].velocity = velocity;
	}
	
	public void setFlipRotation(int idx,boolean inverse){
		wraps[idx].inverse = inverse;
	}
	...
	public WrapProp getProp(int idx){
		return wraps[idx];
	}
