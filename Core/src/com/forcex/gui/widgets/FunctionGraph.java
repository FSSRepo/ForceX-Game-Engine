...
public class FunctionGraph extends View {
	...
	public void onCreate(Drawer drawer) {
		...
		float[] vertexs = new float[(numSteps + 1) * 4];
		...
		vbo = drawer.genBuffer(vertexs,false,false);
		ibo = drawer.genBuffer(indices,true,false);
		indices = null;
		indices = new short[]{0,1};
		ibo_cur = drawer.genBuffer(indices,true,false);
	}
	...
	private void stepCursor(){
		...
		vertexs[4] = (-extent.x) + (cursorX * extent.x * 2.0f);
		vertexs[5] = (-extent.y) + ((y / max) * extent.y * 2f);
		vertexs[6] = 
		vertexs[7] = 0.0f;
		rect_tangent.set(vertexs[4] + local.x,vertexs[5] + local.y);
		if(vbo_cur == -1){
			vbo_cur = Drawer.genBuffer(vertexs,false,true);
		}else{
