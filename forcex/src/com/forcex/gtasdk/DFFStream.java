...
public class DFFStream {
    ...
    private static final int FRAME_LIST = 0xE;
    private static final int GEOMETRY_LIST = 0x1A;
    private static final int GEOMETRY = 0xF;
    private static final int MATERIAL_LIST = 0x8;
    ...
    private static final int ANIM_INFO = 0x11E;
    ...
    private static boolean readSection(BinaryStreamReader is, int sect) {
        int id = is.readInt();
        if (id == sect) {
            is.skip(8);
            return true;
    ...
    private static void skipSection(BinaryStreamReader is, int sect) {
        int id = is.readInt();
        if (id == sect) {
            is.skip(8);
        } else {
    ...
    public static DFFSDK readDFF(String path, OnDFFStreamingListener listener, LanguageString lang, String save_collisions_dir) {
        ...
        try {
            ...
            if (readSection(is, CLUMP)) {
                ...
                if (readSection(is, FRAME_LIST)) {
                    ...
                    if (readSection(is, GEOMETRY_LIST)) {
                        is.skip(12); // struct
                        ...
                        for (int i = 0; i < dff.geometryCount; i++) {
                            if (is.readInt() == GEOMETRY) {
                                ...
                                if ((geo.flags & DFFGeometry.GEOMETRY_FLAG_TEXCOORDS) != 0 || (geo.flags & DFFGeometry.GEOMETRY_FLAG_MULTIPLEUVSETS) != 0) {
                                    geo.uvs = is.readFloatArray(geo.vertexCount * 2 * geo.uvsets);
                                }
                                ...
                                if (readSection(is, MATERIAL_LIST)) {
                                    is.skip(12); //struct
                                    ...
                                    for (int m = 0; m < numMaterials; m++) {
                                        ...
                                        if (readSection(is, MATERIAL)) {
                                            is.skip(12);
                                            is.skip(4);
                                            mat.color = new Color(is.readUnsignedByte(), is.readUnsignedByte(), is.readUnsignedByte(), is.readUnsignedByte());
                                            is.skip(4);
                ...
                if (is.readInt() == EXTENSION) {
                    if (is.readInt() != 0) {
                        ...
                        if (is.readInt() == COLLISION_MODEL) {
                            ...
                            if (save_collisions_dir != null) {
                                // FX.fs.homeDirectory + "zmdl/"+dff.col_id+".col"
                                FileOutputStream col = new FileOutputStream(save_collisions_dir + dff.col_id + ".col");
    ...
    private static void readTexture(DFFMaterial mat, BinaryStreamReader is) {
        is.skip(32); //Texture y Estructura
        int size = is.readInt();
        is.skip(4);
        mat.texture = fixText(GameUtils.trimString(is.readString(size)));
        is.skip(4);
    ...
    public static boolean saveDFF(DFFSDK dff, String path, OnDFFStreamingListener listener, LanguageString lang) {
        try {
            ...
            RWSection framelist = new RWSection(FRAME_LIST, dff.game);
            RWSection framestruct = new RWSection(STRUCT, dff.game);
            framestruct.writeInt(dff.frameCount);
            framestruct.allocate(dff.frameCount * 56);
            if (listener != null) {
            ...
            for (int i = 0; i < dff.frameCount; i++) {
                ...
                if (dff.fms.get(i).hanim != null) {
                    RWSection hanim = new RWSection(ANIM_INFO, dff.game);
            ...
            RWSection geolist = new RWSection(GEOMETRY_LIST, dff.game);
    ...
    private static void writeGeometryList(DFFSDK dff, RWSection list, OnDFFStreamingListener listener, LanguageString lang) {
        ...
        for (int i = 0; i < dff.geometryCount; i++) {
            ...
            if (geo.uvs != null) {
                struct.writeFloatArray(geo.uvs);
            }
    ...
    private static void writeMaterialList(DFFSDK dff, DFFGeometry geo, RWSection geometry) {
        RWSection matList = new RWSection(MATERIAL_LIST, dff.game);
        RWSection struct = new RWSection(STRUCT, dff.game);
        struct.writeInt(geo.materials.size());
        struct.allocate(geo.materials.size() * 4);
    ...
    private static void writeBinMesh(DFFSDK dff, DFFGeometry geo, RWSection extension) {
        ...
        binmesh.allocate(idxCount * 4 + (geo.splits.size() * 2 * 4));
        for (DFFIndices i : geo.splits) {
            idxCount = i.index.length;
            binmesh.writeInt(idxCount);
    ...
    private static short[] generateFaces(DFFGeometry geo) {
        ...
        for (int j = 0; j < geo.splits.size(); j++) {
            for (int i = 0; i < geo.splits.get(j).index.length - 2; i += 3) {
                faces[tri] = geo.splits.get(j).index[i];
    ...
    private static void writeExtensionClump(DFFSDK dff, RWSection clump) {
        ...
        if (dff.hasCollision) {
            ...
            try {
                BinaryStreamReader is = FX.fs.open("zmdl/" + dff.col_id + ".col", FileSystem.ReaderType.STREAM);
                extension.writeInt(is.length());
                col = is.readByteArray(is.length());
                is.clear();
            } catch (Exception e) {
