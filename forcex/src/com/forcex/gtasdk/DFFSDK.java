...
public class DFFSDK {
    ...
    public void updateParents(DFFFrame parent) {
        for (DFFFrame child : parent.children) {
    ...
    public ModelObject getObject(Object obj, int i, boolean forceNormalMap) {
        ...
        for (int j = 0; j < geo.splits.size(); j++) {
            DFFIndices index = geo.splits.get(j);
            DFFMaterial mat = geo.materials.get(index.material);
            MeshPart part = new MeshPart(index.index);
