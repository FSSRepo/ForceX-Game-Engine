...
public class PreLightingSystem
{
	...
	public byte[] applyLighting(ModelObject obj){
		VertexData v = obj.getMesh().getVertexData();
		float[] vertices = v.vertices;
		float[] normals = v.normals;
		Matrix4f mm = obj.getTransform();
		byte[] colors = new byte[(vertices.length/3) * 4];
		int offset = 0;
		for(int i = 0;i < vertices.length;i+=3){
			...
			for(PreLight light : lights){
				...
				tmpcolor.addLocal(light.ambient.add(light.diffuse.mult(diff)).mult(0.5f));
			}
		...
		obj.getMesh().setVertexColor(colors);
		return colors;
	}
}
