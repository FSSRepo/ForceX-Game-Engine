...
public class Cone extends Mesh {
    public Cone(int axisSamples, int radialSamples, float radius,
                float height, boolean closed) {
        super(true);
        height *= 2f;
        float bottomRadius = 0.0001f;
        ...
        for (int circlePoint = 0; circlePoint < radialSamples + 1; circlePoint++) {
            ...
            Vector3f normal = new Vector3f(height * circlePoints[circlePoint][0], bottomRadius - radius, height * circlePoints[circlePoint][1]);
            normal.normalize();
        ...
        for (int axisSample = 0; axisSample < axisSamples; axisSample++) {
            ...
            float currentRadius = bottomRadius + (radius - bottomRadius) * axisSample / (axisSamples - 1);
            for (int circlePoint = 0; circlePoint < radialSamples + 1; circlePoint++) {
                ...
                if (closed)
                    texcoords[currentIndex * 2 + 1] = (bottomRadius + (height * 0.5f) + currentHeight) / (bottomRadius + height + radius);
                else
        ...
        if (closed) {
            ...
            for (int circlePoint = 0; circlePoint < radialSamples + 1; circlePoint++) {
                ...
                texcoords[currentIndex * 2] = (float) circlePoint / radialSamples;
                texcoords[currentIndex * 2 + 1] = bottomRadius / (bottomRadius + height + radius);

                currentIndex++;
            }
            // Top
            for (int circlePoint = 0; circlePoint < radialSamples + 1; circlePoint++) {
                vertices[currentIndex * 3] = circlePoints[circlePoint][0] * radius;
                vertices[currentIndex * 3 + 1] = height * 0.5f;
                vertices[currentIndex * 3 + 2] = circlePoints[circlePoint][1] * radius;
                ...
                texcoords[currentIndex * 2] = (float) circlePoint / radialSamples;
                texcoords[currentIndex * 2 + 1] = (bottomRadius + height) / (bottomRadius + height + radius);
