...
public class VideoClip
{
	...
	private int texture = -1,blockSize,fileOffset,type = 0;
	private float duration,frame_time,speed = 1;
		...
		for(int i = filenStart;i <= filenEnd;i++){
			...
			if(!dxt){
				...
				if(zip.length > blck_sz){
					...
					os.writeByteArray(zip); bitcount += zip.length;
				}
				if(alpha) {
				...
				if(zip.length > blck_sz){
					os.writeByte(0);
					os.writeByteArray(buffer); bitcount += blck_sz;
					ratio = 1;
				}else{
					os.writeByte(1);
					is_zip = true;
					os.writeInt(zip.length);
					ratio = (float)zip.length / blck_sz; bitcount += zip.length;
					os.writeByteArray(zip);
				}
				if(alpha) {
					BitStream bs = new BitStream(img.width * img.height);
	...
	private static byte[] encode(byte[] input) {
		...
		for(int i = 0;i < lenght;i++){buffer[i] = temp[i];}
		return buffer;
	}
	public static interface OnVideoProcessListener{
