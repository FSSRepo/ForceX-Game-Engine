...
public abstract class TreeAdapter {
    ...
    protected float width_total, residual;
    ...
    void create() {
        ...
        residual = treeview.getExtentHeight() / item_height;
    ...
    void render(Drawer drawer) {
        ...
        if (curY < 0.0f || num_items < residual) {
            curY = 0.0f;
        }
        if (num_items > residual && curY > limitY) {
            curY = ((num_items - residual) * item_height2);
        }
        delta.set(
        ...
        if (num_items != old_num) {
            old_num = num_items;
            limitY = ((num_items - residual) * item_height2);
        }
    }
