...
public class KeyBoard {
	...
	public void create(UIContext context) {
		...
		fill('~',':',',',';','"','_','*','\'', '\"','n');
		for(byte i = 0;i < 10;i++){
			keys.get(57 + i).key_pos.set(dx + (i * (key_width * 2.1f)),dy);
		}
		dy -= key_height * 2f;
		// key number
		dy = (position.y + height) - key_height;
	...
	public void onTouch(float x, float y,byte type) {
		if(index != -1 && ref != null && GameUtils.testRect(x,y,ref.local,ref.getExtentWidth(),ref.getExtentHeight())) {
			ref.onTouch(x,y,type);
			return;
		}
		if(!keyboardNumber){
			for(KeyButton btn : buttons){
	...
	public boolean testTouch(float x,float y) {
		...
		if(showKeyBoard && index != -1 && ref != null) {
			ref.local.set(position.x,position.y + height + (ref.getExtentHeight() * 1.2f));
			if(GameUtils.testRect(x,y,ref.local,ref.getExtentWidth(),ref.getExtentHeight())){
				return true;
			}
		}
		return  showKeyBoard && GameUtils.testRect(x,y,position,1.0f,height);
	}
