...
public abstract class GridAdapter <T extends Object>{
	...
	void onTouch(float x,float y,byte type){
		...
		if(type == EventType.TOUCH_PRESSED){
			...
			if(!touchCursor){
				...
				if(curY + delta <= ((num_cols - excedent) * item_height2)){
					curY += delta;
					t_deltaY += delta;
	...
	void render(Drawer drawer){
		...
		if(Maths.abs(t_deltaY) > gridview.getExtentHeight() * 0.08f && touch_time > 0){
			if(delta_first){
				timelapse = Maths.abs(1f - touch_time);
				timelapse = timelapse > 1f ? 1f : timelapse;
				delta_first = false;
				t_deltaY = t_deltaY * touch_time;
			}
			curY += t_deltaY * timelapse * 0.3f;
			if(timelapse > 0){
		...
		if(num_cols > simpleViewVisible && curY > ((num_cols - excedent) * item_height2)){
			curY = ((num_cols - excedent) * item_height2);
		}
		byte curx = 0;
		testCulling();
		for(short i = 0;i < items.size();i++){
	...
	public void setBeginRow(short row){
		...
		float test = (num_cols - excedent) * item_height2;
		if(curY > test){
			curY = test;
		}
	}
