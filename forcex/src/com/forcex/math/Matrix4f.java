...
public class Matrix4f {
    ...
    /* specials */
    Vector3f look_temp = new Vector3f();
    ...
    public void lookAt(Vector3f direction, boolean upZ) {
        ...
        data[M11] = up.y;
        data[M21] = up.z;
        data[M02] = direction.x;
        data[M12] = direction.y;
    ...
    public Matrix4f setlookAt(Vector3f position, Vector3f direction, Vector3f up) {
        direction.cross(up, look_temp).normalize();
        data[M00] = look_temp.x;
        data[M01] = look_temp.y;
        data[M02] = look_temp.z;
        ...
        data[M03] = -look_temp.dot(position);
    ...
    public String toString() {
        ...
        for (byte i = 0; i < 16; i += 4) {
            debug += String.format("%.3f | %.3f | %.3f | %.3f", data[i], data[i + 1], data[i + 2], data[i + 3]) + (i == 12 ? "" : "\n");
        }
        debug += "]";
        return debug;
    }
