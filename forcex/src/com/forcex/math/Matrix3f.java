...
public class Matrix3f {
    ...
    public Vector3f column(int i) {
		return new Vector3f(data[0 + i * 3], data[1 + i * 3], data[2 + i * 3]);
        
    }

    public Vector3f row(int i) {
        return new Vector3f(data[0 + i], data[3 + i], data[6 + i]);
    }
	...
	public String toString() {
		...
		for(byte i = 0;i < 9;i += 3){
			debug += String.format("%.3f", data[i])+" | "+String.format("%.3f", data[i+1])+" | "+String.format("%.3f", data[i+2])+ (i == 6 ? "":"\n");
		}
        debug += "]";
        return debug;
    }
